...
/****************
 ZS, 14/2/15, 18:34
 The exp, m_sine and e_sign were missing a pipeline stage
*****************/
...
//Buffers for flopping incoming data
reg [63:0] v0_r, v1_r, v2_r, v3_r, v4_r, v5_r, v6_r, v7_r, sj_r, sk_r;
...
wire [6:0] instr;
reg [6:0]  instr0, instr1, instr2, instr3, instr4;
...
/*
//flop the incoming data for timing reasons
always@(posedge clk)
   begin
	   v0_r <= i_v0;
		v1_r <= i_v1;
...
// Unit is ready to receive new data two clocks before exiting last processed
...
assign selected_j_src = (reservation_time==7'b1111111) ? sj_r : v_j_src;
assign selected_k_src = (reservation_time==7'b1111111) ? sk_r : v_k_src;
...
always@(posedge clk)
   if(rst)
	    ...
	    reservation_time <= i_vector_length;  
...
assign o_busy = (reservation_time > 7'b1);
*/
...
always@(posedge clk)
begin
    m_sign0 <= selected_j_src_r[63]^selected_k_src_r[63];       //compute sign bit of mantissa - result is only negative if only one of them is negative
    case({selected_j_src_r[62],selected_k_src_r[62],expj_gt_expk})
        3'b000:begin
                exp0    <= selected_j_src_r[61:48] + selected_k_src_r[61:48];               //both negative
                e_sign0 <= 1'b0;
               end
        3'b001:begin 
                exp0    <= selected_j_src_r[61:48] + selected_k_src_r[61:48];               //both negative
        ...
        3'b010:begin                                                 //sj=+, sk=-, sj <= sk
                exp0    <= selected_k_src_r[61:48] - selected_j_src_r[61:48];
                e_sign0 <= 1'b0;                                     //result=-
               end
        3'b011:begin                                                 //sj=+, sk=-, sj > sk
                exp0    <= selected_j_src_r[61:48] - selected_k_src_r[61:48];
                e_sign0 <= 1'b1;                                //result=+
               end
        ...
        3'b110:begin
                exp0 <= selected_j_src_r[61:48] + selected_k_src_r[61:48];               //both positive
                e_sign0 <= 1'b1;
               end
        3'b111:begin
                exp0 <= selected_j_src_r[61:48] + selected_k_src_r[61:48];               //both positive
    ...
    exp3 <= exp2;
    exp4 <= exp3;
	 //FIXME: Figure out how to properly compute this. It's supposed to do an integer multiply if both exponents are zero.
    exp5 <= exp4; //MULT[95] ? exp4 : (e_sign4 ? (exp4) : (exp4 + 14'b1));      //if we need to shift the final answer by 1, readjust the exponents
